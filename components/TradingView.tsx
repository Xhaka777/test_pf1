...
const TradingViewChart = memo(function TradingViewChart(
  ...
  const addPositionLinesToChart = useCallback(() => {
    ...
    openTrades?.open_trades.forEach((openTrade) => {
      ...
      if (openTrade.sl) {
        ...
        SLline?.onMove(async () => {
          ...
          try {
            ...
            toast({
              title: t('Failed to update SL'),
              description: (e as { message: string })?.message ?? undefined,
            });
            SLline.setPrice(openTrade.sl).setQuantity(
              openTrade.trade_loss.toLocaleString('en-US', {
                style: 'currency',
        ...
        /* Commented out to disable X button for SL lines
          .onCancel(async () => {
            if (!SLline) {
              return;
            }
            if (!oneClickTradingEnabled) {
              ...
              if (!answer) {
                SLline.setPrice(openTrade.tp).setQuantity(
            ...
            try {
              const result = await onUpdateSl(null, openTrade);
              if (result.status !== StatusEnum.SUCCESS) {
                toast({
                  title: t('Failed to update SL'),
                  description: result.message,
      ...
      if (openTrade.tp) {
        ...
        /* Commented out to disable X button for TP lines
          .onCancel(async () => {
            if (!TPline) {
              return;
            }
            ...
            try {
              const result = await onUpdateTp(null, openTrade);
              if (result.status !== StatusEnum.SUCCESS) {
                toast({
                  title: t('Failed to update TP'),
                  description: result.message,
      ...
      if (openOrder.tp) {
        ...
        TPline?.onMove(async () => {
          ...
          .onMoving(async () => {
            ...
            // const profit = calculateTpSl(
            //   price,
            //   openOrder.entry,
            //   openOrder.tp,
            //   openOrder.trade_profit,
            // );
            // TPline?.setQuantity(
            //   profit.toLocaleString('en-US', {
            //     style: 'currency',
        ...
        .onCancel(() => {
          const cancel = async (
            ...
            try {
              ...
              if (result.status === StatusEnum.SUCCESS) {
                toast({
                  title: t('Order "{{id}}" canceled', {
                    id: order.order_id,
                  }),
                });
              } else {
                toast({
                  title: t('Failed to cancel order "{{id}}"', {
                    id: order.order_id,
                  }),
                  description: result.message,
                });
  ...
  const initTradingViewWidget = useCallback(() => {
    ...
    const bg = getCssVariableHex('--background') ?? undefined;
