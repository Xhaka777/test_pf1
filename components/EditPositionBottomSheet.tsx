...
const EditPositionBottomSheet = forwardRef<BottomSheetModal, EditPositionBottomSheetProps>(
    ({ openTrade, onSave, onClose }, ref) => {
        const { t } = useTranslation();

        // State management - similar to web version
        const [showSl, setShowSl] = useState<boolean>(
            !!openTrade.sl && openTrade.sl > 0,
        );
        const [showTp, setShowTp] = useState<boolean>(
            !!openTrade.tp && openTrade.tp > 0,
        );
        const [showPartialTP, setShowPartialTP] = useState<boolean>(
            !!openTrade.tp_levels && openTrade.tp_levels.length > 0,
        );
        const [showTrailingSL, setShowTrailingSL] = useState<boolean>(
            !!openTrade.trailing_sl && openTrade.trailing_sl.length > 0,
        );

        // Form values
        const [slValue, setSlValue] = useState<string>(openTrade?.sl?.toString() || '0');
        const [tpValue, setTpValue] = useState<string>(openTrade?.tp?.toString() || '0');
        ...
        // Toast function for React Native (you'll need to implement this based on your toast library)
